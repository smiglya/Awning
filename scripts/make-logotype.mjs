...
/**
 * Assembles the three brand/logo-*.svg variants: the mark, then "wning".
 *
...
/**
 * Clear space is the height of the lowercase n, on all four sides — which is
 * the x-height, the number this generator already sets every lowercase from.
 * Written once here and restated as a ratio in SiteNav.css; both derive from
 * the same measurement rather than from each other.
 */
const CLEAR = round(XH)

const paths = (indent) => drawn.map((d) => `${indent}<path d="${d}" />`).join('\n')

/**
 * logo-flat.svg — the working file, and the one with rules attached.
 *
 * No defs, mask, radialGradient or filter; absolute commands only; fill on the
 * root and nowhere else; viewBox tight to the ink from 0 0. Everything
 * downstream depends on those: the navigation sets its colour by inheritance,
 * the prerender inlines it, and scripts/lib/svg-raster.mjs — which is ours, not
 * a browser — supports none of the heavy nodes. A gradient here does not
 * degrade, it disappears. tests/brand.test.ts enforces the whole list.
 */
const flat = `<!-- Awning logotype, flat: the mark, then the name as drawn letters.
     ...
     src/components/icons.tsx redraws that bit as pixel art, module for module.

     This is the variant for navigation, the prerender and our own rasteriser:
     no defs, no mask, no gradient, no filter, no stroke, absolute commands
     only. viewBox is tight to the ink so no empty space travels with the
     artwork, and fill is currentColor so it reverses out of a dark surface. -->
<svg
...
/**
 * logo-full.svg — the same drawing carrying its own clear space.
 *
 * For the Open Graph card and any marketing surface where nothing on our side
 * controls the padding. The flat file is tight to the ink on purpose, and a
 * tight box dropped onto someone else's layout gets crowded; this one cannot
 * be, because the margin is inside the artwork.
 */
const full = `<!-- Awning logotype, full: the flat drawing with its clear space built in.

     Generated by scripts/make-logotype.mjs — edit that, not this.

     Identical geometry to logo-flat.svg, offset by one lowercase-n height on
     all four sides. Use this where the layout is not ours — the Open Graph
     card, a partner deck, anything that would otherwise set the lockup flush
     against an edge. Use logo-flat.svg everywhere our own CSS owns the box. -->
<svg
  xmlns="http://www.w3.org/2000/svg"
  viewBox="0 0 ${round(width + CLEAR * 2)} ${round(height + CLEAR * 2)}"
  width="${round(width + CLEAR * 2)}"
  height="${round(height + CLEAR * 2)}"
  fill="currentColor"
>
 ...
 *
 * The other two inherit `color`, which is the right answer wherever CSS
 * reaches. It does not reach an email signature, a PDF, or an <img> tag, and in
 * all three the inheriting file renders ink-on-ink and looks like nothing at
 * all. So this one carries both colours itself, and they are palette values —
 * #0A0A0A and #FAFAF9 — not a black and a white invented for the occasion.
 *
 * The mark is never orange. Orange on this site means "press this", and the
 * logo is not a button.
 */
const invert = `<!-- Awning logotype, reversed: paper artwork on an ink plate.

     Generated by scripts/make-logotype.mjs — edit that, not this.

     The only variant that names its own colours, because it is the one for
     places CSS cannot reach: an <img> tag, an email signature, a PDF. Both
     values are from the palette — #0A0A0A is --ink, #FAFAF9 is --paper. The
     mark is never set in orange, here or anywhere. -->
<svg
  xmlns="http://www.w3.org/2000/svg"
  viewBox="0 0 ${round(width + CLEAR * 2)} ${round(height + CLEAR * 2)}"
  width="${round(width + CLEAR * 2)}"
  height="${round(height + CLEAR * 2)}"
>
  <rect width="${round(width + CLEAR * 2)}" height="${round(height + CLEAR * 2)}" fill="#0A0A0A" />
...
console.log(
  `brand/logo-flat.svg        ${width} x ${height}  ${drawn.length} paths` +
    `  (cap ${round(CAP)}, x-height ${XH}, stem ${S})`
)
console.log(
  `brand/logo-full.svg        ${round(width + CLEAR * 2)} x ${round(height + CLEAR * 2)}` +
    `  (clear space ${CLEAR}, one lowercase n)`
)
console.log(`brand/logo-invert.svg      same box, paper on ink`)
console.log(
